...
# (Insert right after train_dataset = RewardDataset(...))
...
inspect("state",        sample["state"])
inspect("state_mask",   sample["state_mask"])
inspect("action",       sample["action"])
inspect("action_mask",  sample["action_mask"])
inspect("target_reward",sample["target_reward"])
# if you also have seg. maps:
if "segmentation_target" in sample:
    inspect("seg_target", sample["segmentation_target"])
...
print(f"Now trainable: {trainable} / {total} "
